...
export const createSuperAdmin = async (req, res, next) => {
    try {
        ...
        const data = { firstName, lastName, mobile, email, password: encryptedPassword }
        const userExist = await superAdminModel.findOneAndUpdate({ email: email }, { $set: { isActive: true } })
        if (userExist) {
            return res.status(200).send({ data: { message: "superadmin exist already" }, status: "ok" })
        }
        const user = await superAdminModel.create(data)
        if (user == null) {
            return res.status(400).send({ message: "failed to create superadmin", status: "fail" })
        }
        user.password = null
        const profilePic = `/public/superadmin/${user.id.slice(-6)}${file.originalname}`
        const uploadPic = await superAdminModel.findByIdAndUpdate({ _id: user._id }, { $set: { profilePic: profilePic } })
        await fs.writeFile(appDir + `../../public/superadmin/${user.id.slice(-6)}${file.originalname}`, imgBuffer, "utf-8")


        return res.status(201).send({ data: { message: "superadmin created successfully" }, status: "ok" })
    } catch (err) {
...
export const loginSuperAdmin = async (req, res, next) => {
    try {
        ...
        if (user == null) {
            return res.status(400).send({ data: { message: "email doesnt exist. Please register" }, status: "fail" })
        }
        const isPasswordCorrect = await bcrypt.compare(password, user.password);
        if (!isPasswordCorrect) {
            return res.status(400).send({ data: { message: "Incorrect password. Please try again" }, status: "fail" })
        };
        const data = { firstName: user.firstName, lastName: user.lastName, mobile: user.mobile, email: user.email };
...
export const updateSuperAdmin = async (req, res, next) => {
    try {
        ...
        const previousImage = await fs.writeFile(appDir + `../../${user.profilePic}`, imgBuffer, "utf-8")
        const profilePic = `${user.profilePic}`
        const userupdate = await superAdminModel.updateOne({ _id: req.id }, { $set: { profilePic: profilePic } })
...
export const deleteSuperAdmin = async (req, res, next) => {
    try {
        ...
        if (!(email == user.email && bcrypt.compare(password, user.password))) {
            return res.status(400).send({ message: 'please enter correct password and email' })
        }
...
export const updatecenterAdmin = async (req, res, next) => {
    try {
        ...
        const update = await centerAdminModel.updateOne({ _id: id }, { $set: { updateField, updateValue } })
        const file = req.files[0];
        let user
        if (file) {
            const imgBuffer = Buffer.from(file.buffer, "utf-8")
            user = await superAdminModel.findById(req.id)
            const previousImage = await fs.writeFile(appDir + `../../${user.profilePic}`, imgBuffer, "utf-8")
            const profilePic = `${user.profilePic}`
            const userupdate = await superAdminModel.updateOne({ _id: req.id }, { $set: { profilePic: profilePic } })
        }
...
//         }
//         const userdeleted = await centerAdminModel.updateOne({ _id: req.id }, { $set: { isActive: false } })
//         return res.status(200).send({ data: user, status: "ok" })
//     } catch (err) {
//         res.status(500).send({ message: err.message, status: "fail" })
//     }
